...
def build_daily_frame(market_chart_json: dict) -> pd.DataFrame:
    ...
    # Make daily: keep last observation per day (CoinGecko may return multiple points depending on plan/granularity)
    df["day"] = df["date"].dt.floor("D")
    df = df.groupby("day", as_index=False).last().rename(columns={"day": "date"}).drop(columns=["date_y"], errors="ignore")
...
def add_features(fun_df: pd.DataFrame, btc_df: pd.DataFrame | None = None, eth_df: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    Heavy feature engineering:
    - log returns, rolling volatility/momentum
    - volume/mcap transforms
    - optional exogenous BTC/ETH returns & vol
    Target will be next-day log return of FUN.
    """
    df = fun_df.copy()
    ...
    def _merge_exog(exog_df: pd.DataFrame, prefix: str) -> pd.DataFrame:
        ex = exog_df.copy()
        ...
        ex = ex[["date", f"{prefix}_ret1", f"{prefix}_ret_std_14"]]
        return df.merge(ex, on="date", how="left")

    if btc_df is not None:
        df = _merge_exog(btc_df, "btc")
    if eth_df is not None:
        df = _merge_exog(eth_df, "eth")
